...
void tranformation_helpers_write_image(const char *file_name, 
                                       void *buffer, 
                                       size_t bufferSize)
{
    //void buffer = k4a_image_get_buffer(image_to_save);
    //size_t bufferSize = k4a_image_get_size(image_to_save);

    std::cout << bufferSize << std::endl;
    //assert(buffer != NULL);
    std::ofstream hFile;
    hFile.open(file_name, std::ios::out | std::ios::trunc | std::ios::binary);
    if (hFile.is_open())
    {
        hFile.write((char *)buffer, static_cast<std::streamsize>(bufferSize));
        hFile.close();
    }
    std::cout << "[Streaming Service] Color frame is stored in " << file_name << std::endl;    
}
...
#define PLY_ELEMENT_VERTEX "element vertex"
    ...
    std::cout << "Point cloud data successfully stored in " << file_name << std::endl;
}
